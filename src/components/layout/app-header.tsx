...
export function AppHeader({ children, showUpgradeButton = true, isProPlan = false }: AppHeaderProps) {
  ...
  useEffect(() => {
    // Load API keys
    const storedKeys = localStorage.getItem('Resumify-api-keys');
    if (storedKeys) {
      try {
    ...
    // Load default model
    const storedModel = localStorage.getItem('Resumify-default-model');
    if (storedModel) {
      ...
      localStorage.setItem('Resumify-default-model', defaultModelId);
    }
  }, [isProPlan]);

  const handleModelChange = (modelId: string) => {
    setDefaultModel(modelId);
    localStorage.setItem('Resumify-default-model', modelId);
  };
